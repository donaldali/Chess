...
class ChessGame
  ...
  def get_player_names
    ...
    while @players.last.name.nil? || @players.first.name == @players.last.name
  ...
  def check_for_check
    if @chessboard.check?(@cur_player.color)
      Chess.warn_message("WARNING: #{@cur_player.name}<#{@cur_player.color}>, your King is in check")
    end
  end

  # Display an error message for bad input and go back to get new user input
  def invalid message
    Chess.error_message "#{@cur_player.name}<#{@cur_player.color}>, #{message}"
    throw :invalid
  end
  ...
  def get_input
    check_for_check
    prompt = "#{@cur_player.name}<#{@cur_player.color}>, Enter a move (or 'o' for options)"
    loop do 
  ...
  def set_in_game_options
    ...
    options_title = "In Game Options for #{@cur_player.name}<#{@cur_player.color}>"
